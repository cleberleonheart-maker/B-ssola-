...
import { buildEmptyContext, type AssistantContextData, type ActionResult } from './types';
...
export class KeferaEngine {
  ...
  private response(text: string, kind: ActionResult['kind']): ActionResult {
    pushHistory(this.memory, 'assistant', text);
    return { text, speak: true, kind };
  }
